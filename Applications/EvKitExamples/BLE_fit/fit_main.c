...
*/
/*************************************************************************************************/
static dmSecKey_t *fitGetPeerKey(dmEvt_t *pMsg)
{
  appConnCb_t *pCb;
...
*/
/*************************************************************************************************/
static void fitPrivAddDevToResListInd(appDbHdl_t dbHdl)
{
...
*/
/*************************************************************************************************/
static void fitPrivRemDevFromResListInd(fitMsg_t *pMsg)
{
...
static void fitProcCccState(fitMsg_t * pMsg)
{
  ...
  if (pMsg->ccc.idx == FIT_BATT_LVL_CCC_IDX) {
    if (pMsg->ccc.value == ATT_CLIENT_CFG_NOTIFY) {
      BasMeasBattStart((dmConnId_t) pMsg->ccc.hdr.param,
          FIT_BATT_TIMER_IND, FIT_BATT_LVL_CCC_IDX);
    } else {
...
//  AppDbNvmDeleteAll();   // Uncomment to delete the NonVolatile RAM of all the Bonding Data.  
  ...
  if ((connId = AppConnIsOpen()) != DM_CONN_ID_NONE) {
    ...
    case APP_UI_BTN_2_MED:
      ...
      if (!(fitHrmFlags & (CH_HRM_FLAGS_SENSOR_DET |
...
static void fitProcMsg(fitMsg_t * pMsg)
{
  ...
  case DM_SEC_COMPARE_IND:
    ...
    case DM_PRIV_ADD_DEV_TO_RES_LIST_IND:
	  fitPrivAddDevToResListInd(AppDbGetHdl((dmConnId_t) pMsg->hdr.param));	  
      break;	
