...
#if defined(__GNUC__) || DOXYGEN
/*! Empty advertising data to conserve power while advertising */
// Note that empty arrays are only supported by GNUC as a non-standard extension.
...
/*************************************************************************************************/
/*!
 *  \brief  Handle GPIO interrupt message.
 *
 *  \param  pMsg  Fit callback event
 *
 *  \return None.
 */
/*************************************************************************************************/
static void HandleUserEvent(fitMsg_t *pMsg)
{
    /* Indicate on EvKit that GPIO event is being handled. */
    SetUserLED(TRUE);

    /*  Simulate GPIO event processing with a delay. Use interrupt based APIs to prevent the handler from
        being starved */
    mxc_delay(USER_EVENT_DELAY);

    /* Indicate on EvKit that GPIO event handling has been completed. */
    SetUserLED(FALSE);
}

/*************************************************************************************************/
/*!
 *  \brief  Service user defined GPIO interrupt.
 *
 *  \param  cbdata  Data provided when this GPIO interrupt was registered.
 *
 *  \return None.
 */
/*************************************************************************************************/
void HandleUserEventInterrupt(void *cbdata)
{
    ...
    /* Send a FIT_GPIO_IND message to the fit application handler */
    if ((pMsg = WsfMsgAlloc(sizeof(wsfMsgHdr_t))) != NULL) {
        pMsg->hdr.event = FIT_GPIO_IND;
...
#ifdef __GNUC__
    // Zero length arrays are supported by GNUC, so use that in adv data definition if available.
    AppAdvSetData(APP_ADV_DATA_DISCOVERABLE, sizeof(fitAdvDataDisc), (uint8_t *) fitAdvDataDisc);
#else /* __GNUC__ */
    // Otherwise, explicitly set adv data to be NULL.
    AppAdvSetData(APP_ADV_DATA_DISCOVERABLE, 0, (uint8_t *) 0);
#endif /* __GNUC__ */
