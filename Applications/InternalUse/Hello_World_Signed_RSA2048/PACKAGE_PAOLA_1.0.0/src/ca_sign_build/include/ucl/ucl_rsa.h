 ...
 * for keys length=2048 or 3072, it applies section B3.3
 ...
 * @ingroup UCL_RSA
 */
int ucl_rsa_crt_param_gen_legacy(u32 *n, u32 *p, u32 *q, u32 *dp, u32 *dq, u32 *qInv,
                          u32 *e, u32 t, u32 s);
...
/** <b> RSA CRT Parameters Generation</b>.
 * generate RSA keys pairs with FIPS 186-4 compliance.
 * for keys length=1024, it applies section B3.3, otherwise it applies section B.3.6.
 * Generate @a p, @a q, @a n , @a dp, @a dq and @a qInv such as: @n
 * @f$ n = p \times q@f$ @n
 * @f$ e \times dp = 1 \bmod (p-1)@f$ @n
 * @f$ e \times dq = 1 \bmod (q-1)@f$ @n
 ...
 * @ingroup UCL_RSA
 */
  int ucl_rsa_crt_param_gen(u32 *n, u32 *d, u32 *p, u32 *q, u32 *dp, u32 *dq, u32 *qInv,
                          u32 *e, u32 t, u32 s);
 ...
 * @ingroup UCL_RSA
 */
  int ucl_rsa_param_generation_fips186_4(u8 *n, u8 *p, u8 *q, u8 *d, u8 *e, int t, int modulus_bytes_size);
 ...
 * @ingroup UCL_RSA
 */
  int ucl_rsa_crt_param_generation_fips186_4(u8 *n, u8 *d,u8 *p, u8 *q, u8 *dp, u8 *dq, u8 *qInv, u8 *e, u32 t, u32 modulus_bytes_size);
 ...
 * @ingroup UCL_RSA
 */
  int ucl_rsa_crt_param_generation(u8 *n, u8 *d,u8 *p, u8 *q, u8 *dp, u8 *dq, u8 *qInv, u8 *e, u32 t, u32 modulusbytes_size);
 ...
 * @retval #UCL_OK     if no error occurred
 * @retval #UCL_INVALID_OUTPUT  if the output is the pointer #NULL
 * @retval #UCL_INVALID_INPUT  if one of the input is the pointer #NULL, or if the public exponent size is too large
 ...
 * @retval #UCL_OK
 *
 * @warning the public exponent value 65537 is the only one supported
 *
 * @ingroup UCL_RSA
 */

int ucl_rsa_recover_primes_from_secret_exponent(u8 *p,u8 *q,u8 *n,u8 *d,u8 *e,int modulus_bytes_size);
