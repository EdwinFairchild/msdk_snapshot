...
enum
{
  ...
  DATC_DISC_WDXC_FILE,     /*! ARM Ltd. Wireless Data Exchange service */
#endif /* WDXC_INCLUDED */
...
static void datcWdxcFtcCallback(dmConnId_t connId, uint16_t handle, uint8_t op, uint8_t status)
{
  ...
  else if (op == WDX_FTC_OP_EOF)
  {
    if(handle == WDX_FLIST_HANDLE) {
      ...
      APP_TRACE_INFO0(">>> File transfer complete <<<");
      APP_TRACE_INFO2("FILE_SIZE = %d usec = %d", FILE_SIZE, usec);
      APP_TRACE_INFO1("%d Bps", ((FILE_SIZE*1000)/(usec / 1000)));
      datcCb.sendingFile[connId - 1] = FALSE;
    }
  } else if (op == WDX_FTC_OP_VERIFY_RSP) {
    APP_TRACE_INFO1(">>> Verify complete status: %d <<<", status);
  ...
  if (numConnections > 0)
  {
    switch (btn)
    {
      ...
      case APP_UI_BTN_2_MED:
        if ((datcCb.discState[connId - 1] > DATC_DISC_WDXC_FILE) && 
          (datcCb.sendingFile[connId - 1] == FALSE))
        {
