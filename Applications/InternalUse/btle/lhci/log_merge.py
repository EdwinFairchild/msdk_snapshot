...
if len(sys.argv) != 5:
	print "Usage log_merge.py hci_raw.log hci.log <com_target> <output>"
	sys.exit(0)

hci_raw = sys.argv[1]
hci = sys.argv[2]
com_target = sys.argv[3]
output = sys.argv[4]
...
with open(hci) as fp:
    for line in fp:
        ...
        if((com_target in line) and (com_match == 0)):
            temp_line=line
            com_match=1
        # New command or event after save COM
        elif((com_target in line) and (com_match == 1)):
