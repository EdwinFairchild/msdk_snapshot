...
#define LL_MEMORY_FOOTPRINT     0xeed
// #define LL_MEMORY_FOOTPRINT     0x611
...
const BbRtCfg_t _bb_cfg = {
    ...
    /*schSetupDelayUsec*/       BB_SCH_SETUP_DELAY_US -300
};
...
void StackInitSh(void)
#else /* __CROSSWORKS */
void StackInit(void)
#endif /* __CROSSWORKS */
{
    /* Initialize low level SDMA runtime parameters. */
    ...
    // memUsed = LlInitControllerExtInitSh(&ll_init_cfg);
    // memUsed = LlInitControllerInitSh(&ll_init_cfg);
    LlInitControllerInitSh(&ll_init_cfg);
...
void StackInit(void)
{
    _sdma_diag.pfnLastShCall = ((size_t)&StackInitSh << 1);
    StackInitSh();
}
#endif /* __CROSSWORKS */
