...
{
  {30000,  15000,     0},                     /*! Advertising durations in ms, 0 == forever */
  {   96,    200,   400}                      /*! Advertising intervals in 0.625 ms units */
};
...
{
#if 0
  APP_TRACE_INFO3("pEvt->hdr (0x%04x 0x%02x 0x%02x) ", pEvt->hdr.param, pEvt->hdr.event, pEvt->hdr.status);
  APP_TRACE_INFO3("    ->hdr (0x%08x 0x%04x 0x%04x ", pEvt->pValue, pEvt->valueLen, pEvt->handle);
  APP_TRACE_INFO2("           %u MTU = 0x%04x)", pEvt->continuing, pEvt->mtu);
#endif
  if (pEvt->hdr.status != 0) {
    APP_TRACE_INFO3("pEvt->hdr (0x%04x 0x%02x 0x%02x) ", pEvt->hdr.param, pEvt->hdr.event, pEvt->hdr.status);
    if (pEvt->hdr.status == ATT_ERR_MTU_EXCEEDED) {
      APP_TRACE_INFO0("Error: MTU exceeded");
    }
  }
}
...
{
#if 0
  DmDevWhiteListAdd(DM_ADDR_PUBLIC, (bdAddr_t){0x00,0x00,0x33,0x44,0x55,0x66});
...
static void datsProcMsg(dmEvt_t *pMsg)
{
  ...
  switch(pMsg->hdr.event)
  {
    ...
    case DM_CONN_OPEN_IND:
      LED_On(0); /* Blue Connected LED */
      //LED_Off(2); /* Green */
      uiEvent = APP_UI_CONN_OPEN;
      break;

    case DM_CONN_CLOSE_IND:
      LED_Off(0); /* Blue Connected LED */
...
{
#ifdef BLE_ADDR_LSB
    uint8_t bdAddr[6] = {BLE_ADDR_LSB, 0x42, 0x42, 0x42, 0x42, 0xC0};
#else
    // Zach's board
    uint8_t bdAddr[6] = {0x3e, 0x3f, 0x40, 0x41, 0x42, 0x0C};
#endif
    
    switch (event) {
	case APP_UI_RESET_CMPL:
	    APP_TRACE_INFO1("Setting BLE address, LSB == 0x%02x", bdAddr[0]);
...
void DatsStart(void)
{
  ...
  SvcCoreAddGroup();
  SvcWpCbackRegister(NULL, process_msg);
  SvcWpAddGroup();
