...
uint16_t BbBleInitPeriodicList(uint8_t numEntries, uint8_t *pFreeMem, uint32_t freeMemSize)
{
  _sdma_diag.pfnLastShCall = ((size_t)&BbBleInitPeriodicListSh << 1);
  return BbBleInitPeriodicListSh(numEntries, pFreeMem, freeMemSize);
}
#endif /* __CROSSWORKS */
...
bool_t BbBlePeriodicListCheckAddr(uint8_t addrType, uint64_t addr, uint8_t SID)
{
  ...
  for (i = 0; i < bbBlePeriodicListNumEntries; i++)
  {
