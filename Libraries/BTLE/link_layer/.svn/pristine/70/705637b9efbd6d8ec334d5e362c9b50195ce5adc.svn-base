 ...
 */
/*************************************************************************************************/
static uint32_t lctrCalcPerm(uint32_t v)
{
  uint32_t mask;
 ...
 */
/*************************************************************************************************/
static uint16_t lctrCalcMAM(uint16_t a, uint16_t b)
{
  /* (17 x a + b) mod 2^16 */
...
static lctrAdvSet_t *lctrAllocAdvSet(uint8_t handle)
{
  ...
  for (i = 0; i < pLctrRtCfg->maxAdvSets; i++)
  {
    ...
    if (!pAdvSet->enabled)
    {
      ...
      pMsg = (lctrMsgHdr_t *)&pAdvSet->tmrAdvDur.msg;
...
uint8_t lctrPeriodicSelectNextChannel(lctrChanParam_t *pChanParam, uint16_t eventCounter)
{
  ...
  prn = lctrCalcMAM(prn, pChanParam->chIdentifier);
  /* uint16_t prn_s = prn; */
  prn_e = prn ^ pChanParam->chIdentifier;

  /* unmappedChannel */

  unmapChan = LL_MATH_MOD_37(prn_e & 0xFFFF);
...
/*************************************************************************************************/
/*!
 *  \brief      Protected Extended advertising reset handler.
 *
 *  \return     None.
 */
/*************************************************************************************************/
...
static void lctrSlvExtAdvResetProtectedSh(void)
#else /* __CROSSWORKS */
static void lctrSlvExtAdvResetProtected(void)
#endif /* __CROSSWORKS */
{
  BbBleAdvSlaveInit();
...
static void lctrSlvExtAdvResetProtected(void)
{
    _sdma_diag.pfnLastShCall = ((size_t)&lctrSlvExtAdvResetProtectedSh << 1);
    lctrSlvExtAdvResetProtectedSh();
}
#endif /* __CROSSWORKS */
...
uint8_t lctrSlvExtAdvBuildOp(lctrAdvSet_t *pAdvSet, uint32_t maxStartMs)
{
  ...
  if (pAdvSet->param.advEventProp & LL_ADV_EVT_PROP_LEGACY_ADV_BIT)
  {
    ...
    {
      pAdvSet->param.priAdvInterMin = BB_BLE_TO_BB_TICKS(LL_DIR_ADV_INTER_TICKS);
  ...
  if (pLctrSlvExtAdvMsg->enable.durMs)
  {
    ...
    maxTime = BB_US_TO_BB_TICKS(maxStartMs * 1000);
  ...
  if (pAdvSet->pExtAdvAuxPtr)
  {
    ...
    /* Reaload the Bod with the udpated AUX pointer */
    SchReload(pOp);
  }
...
void lctrSlvAuxNonConnNonScanBuildOp(lctrAdvSet_t *pAdvSet)
{
  ...
  pBle = &pAdvSet->auxBleData;
  pAuxAdv = &pBle->op.slvAuxAdv;
...
uint8_t LctrIsExtAdvEnableReady(uint8_t handle)
{
  ...
  /* Allow empty scan response data when using legacy advertising */
  if (!(pAdvSet->param.advEventProp & LL_ADV_EVT_PROP_LEGACY_ADV_BIT) &&
...
#else
  // Crossworks doesn't like the 64 bit switch value
  if(((bdAddr & LL_RAND_ADDR_TYPE_MASK) != LL_RAND_ADDR_TYPE_STATIC) &&
    ((bdAddr & LL_RAND_ADDR_TYPE_MASK) != LL_RAND_ADDR_TYPE_RPA) && 
    ((bdAddr & LL_RAND_ADDR_TYPE_MASK) != LL_RAND_ADDR_TYPE_NRPA)) {
...
{
  uint32_t offsetUsec, anchorPoint;
  ...
  offsetUsec = SchRmGetOffsetUsec(rsvnOffs, BB_TICKS_TO_US(pAdvSet->perParam.perAdvInter),
  ...
  while (TRUE)
  {
    uint32_t perInterUsec, perInter;
    int16_t dueOffsetUsec;
    ...
    perInterUsec = numIntervals * BB_TICKS_TO_US(pAdvSet->perParam.perAdvInter);
    perInter     = BB_US_TO_BB_TICKS(perInterUsec);
    dueOffsetUsec  = perInterUsec - BB_TICKS_TO_US(perInter);
...
{
  uint32_t interMinUsec, interMaxUsec, durUsec, perInterUsec;
  ...
  /*** Commit operation ***/
  SchBleCalcAdvOpDuration(pOp);
  interMinUsec = BB_TICKS_TO_US(pAdvSet->perParam.advInterMin);
  interMaxUsec = BB_TICKS_TO_US(pAdvSet->perParam.advInterMax);
  durUsec = pOp->minDurUsec;
...
void lctrGetPerAdvOffsets(uint32_t rsvnOffs[], uint32_t refTime)
{
  ...
  for (perIndex = 0; perIndex < pLctrRtCfg->maxAdvSets; perIndex++)
  {
