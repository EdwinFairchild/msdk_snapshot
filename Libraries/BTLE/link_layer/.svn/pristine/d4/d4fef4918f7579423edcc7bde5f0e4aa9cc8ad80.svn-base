...
void bbTestRxCompCback(uint8_t status, int8_t rssi, uint32_t crc, uint32_t timestamp, uint8_t rxPhyOptions)
{
  ...
  bool_t bodComplete = FALSE;
  uint32_t pktInterUsec, pktInter;
  ...
  else
  {
    ...
    pktInterUsec  = pBbRtCfg->rfSetupDelayUs;
    pktInter      = BB_US_TO_BB_TICKS(pktInterUsec);
    dueOffsetUsec = pktInterUsec - BB_TICKS_TO_US(pktInter);
...
void BbBleTestInit(void)
{
  _sdma_diag.pfnLastShCall = ((size_t)&BbBleTestInitSh << 1);
  BbBleTestInitSh();
}
#endif /* __CROSSWORKS */
