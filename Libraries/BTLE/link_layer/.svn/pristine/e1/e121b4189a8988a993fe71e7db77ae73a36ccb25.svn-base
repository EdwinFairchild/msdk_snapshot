...
#ifdef __CROSSWORKS
// TODO: This is also used in wakeup.c. Should be able to be set by the SDMA application.
...
#ifdef __CROSSWORKS
// TODO: Remove. This should not be accessed directly by link layer.
sfrw  r_intout = 0x60;
#endif /* __CROSSWORKS */
...
{
  uint32_t txWinOffsetUsec, txWinOffset, txWinSizeUsec, wwTxWinOffsetUsec, wwTxWinOffset;
  ...
  txWinOffsetUsec    = LCTR_CONN_IND_US(pConnUpdInd->txWinOffset);
  txWinOffset        = BB_US_TO_BB_TICKS(txWinOffsetUsec);
  txWinSizeUsec      = LCTR_CONN_IND_US(pConnUpdInd->txWinSize);
  wwTxWinOffsetUsec  = lctrCalcIntervalWindowWideningUsec(pCtx, txWinOffsetUsec);
  wwTxWinOffset      = BB_US_TO_BB_TICKS(wwTxWinOffsetUsec);
...
void lctrSlvConnEndOp(BbOpDesc_t *pOp)
{
  ...
  numUnsyncIntervals = pCtx->eventCounter - pCtx->data.slv.lastActiveEvent + 1;
  ...
  while (TRUE)
  {
    uint32_t unsyncTimeUsec, wwTotalUsec, wwTotal, connInterUsec, connInter;
    uint16_t dueOffsetUsec;

    pCtx->eventCounter += 1;
    numUnsyncIntervals += 1;

    unsyncTimeUsec = LCTR_CONN_IND_US(pCtx->connInterval * numUnsyncIntervals);

    /* Need to add the unsynced time before connection update. */
    unsyncTimeUsec += pCtx->data.slv.unsyncedTime;

    wwTotalUsec    = lctrCalcIntervalWindowWideningUsec(pCtx, unsyncTimeUsec);
    wwTotal        = BB_US_TO_BB_TICKS(wwTotalUsec);
    connInterUsec  = LCTR_CONN_IND_US(numUnsyncIntervals * pCtx->connInterval);
    connInter      = BB_US_TO_BB_TICKS(connInterUsec);
    dueOffsetUsec  = (connInterUsec - wwTotalUsec) - BB_TICKS_TO_US(connInter - wwTotal);
