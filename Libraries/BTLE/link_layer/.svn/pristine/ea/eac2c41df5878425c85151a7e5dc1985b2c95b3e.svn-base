...
/*************************************************************************************************/
/*!
 *  \brief      Protected Slave advertising reset handler.
 *
 *  \return     None.
 */
/*************************************************************************************************/
...
#endif /* __CROSSWORKS */
static void lctrSlvAdvResetHandlerProtected(void)
{
  BbBleAdvSlaveInit();
  LctrSlvAdvDefaults();
  LmgrSlvInit();
...
static void lctrSlvAdvResetHandler(void)
{
  ...
  lctrSlvAdvResetHandlerProtected();
...
void lctrSlvRxScanReq(void)
{
  ...
  while ((pScanReport = WsfMsgDeq(&lctrSlvAdv.rxScanReqQ, &handlerId)) != NULL)
  {
    ...
    evt.hdr.param  = 0;
    evt.hdr.event  = LL_SCAN_REPORT_IND;
    evt.hdr.status = LL_SUCCESS;

    evt.peerAddr     = pScanReport->scanIdAddr;
    evt.peerAddrType = pScanReport->scanIdAddrType;
    evt.peerRpa      = 0;
...
void lctrSlvAdvBuildOp(void)
{
  ...
  pOp = &lctrSlvAdv.advBod;
  pBle = &lctrSlvAdv.bleData;
  pAdv = &pBle->op.slvAdv;
  ...
  pBuf = lctrSlvAdv.advBuf + LL_ADV_HDR_LEN;
  switch (lmgrSlvAdvCb.advParam.advType)
  {
