...
static bool_t bbSetupAdvOp(BbOpDesc_t *pBod, BbBleSlvAdvEvent_t *pAdv, uint8_t status, bool_t firstOpInSet)
{
  ...
  desc.pBuf = pAdv->pTxAdvBuf;
  desc.len = pAdv->txAdvLen;
    ...
    case BB_EVT_STATE_RX_SCAN_OR_CONN_IND:
      switch (status)
      {
        case BB_STATUS_SUCCESS:
        {
          ...
          pduAllow = BbBlePduFiltCheck(pAdv->pRxReqBuf, &pBle->pduFilt, FALSE, &pAdv->filtResults);
...
void BbBleAdvSlaveInit(void)
{
  _sdma_diag.pfnLastShCall = ((size_t)&BbBleAdvSlaveInitSh << 1);
  BbBleAdvSlaveInitSh();
}
#endif /* __CROSSWORKS */
