...
uint32_t lctrMstConnAdjustOpStart(lctrConnCtx_t *pCtx, uint32_t scanRefTime, lctrConnInd_t *pConnInd)
{
  ...
  uint32_t firstCeDue, anchorPoint, maxOffsetUsec, connInterUsec, connInter;
  ...
  maxOffsetUsec = SchRmGetOffsetUsec(rsvnOffs, LCTR_CONN_IND_US(pConnInd->interval), LCTR_GET_CONN_HANDLE(pCtx));
  ...
  while (TRUE)
  {
    ...
    connInterUsec   = LCTR_CONN_IND_US(numIntervals * pCtx->connInterval);
    connInter       = BB_US_TO_BB_TICKS(connInterUsec);
    dueOffsetUsec   = connInterUsec - BB_TICKS_TO_US(connInter);
