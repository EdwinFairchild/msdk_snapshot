...
void lctrSlvConnBuildOp(lctrConnCtx_t *pCtx)
{
  ...
  uint16_t  txWinDelay, txWinOffsetCnt, dueOffsetUsec;
  uint32_t fastTermCnt, txWinOffsetUsec, txWinOffset, txWinSizeUsec, wwOffsetUsec,
    wwOffset, ceDurUsec, ceSyncDlyUsec, unsyncTimeUsec, unsyncTime, 
    wwTotalUsec, wwTotal;
  ...
  txWinOffsetCnt = pConnInd->txWinOffset + txWinDelay;
  ...
  fastTermCnt = txWinOffsetCnt + pConnInd->txWinSize +
  ...
  pCtx->txHdr.llid = (uint8_t) ~LL_LLID_VS_PDU;     /* reset last PDU LLID */
  ...
  txWinOffsetUsec = LCTR_CONN_IND_US(txWinOffsetCnt);
  txWinOffset     = BB_US_TO_BB_TICKS(txWinOffsetUsec);
  txWinSizeUsec   = LCTR_CONN_IND_US(pConnInd->txWinSize);
  wwOffsetUsec    = lctrCalcIntervalWindowWideningUsec(pCtx, txWinOffsetUsec);
  wwOffset        = BB_US_TO_BB_TICKS(wwOffsetUsec);
  dueOffsetUsec   = (txWinOffsetUsec - wwOffsetUsec) - BB_TICKS_TO_US(txWinOffset - wwOffset);
  ...
  ceDurUsec = pOp->minDurUsec;
  ceSyncDlyUsec = pConn->rxSyncDelayUsec;
  ...
  while (TRUE)
  {
    ...
    unsyncTimeUsec = LCTR_CONN_IND_US(pCtx->connInterval * pCtx->eventCounter);
