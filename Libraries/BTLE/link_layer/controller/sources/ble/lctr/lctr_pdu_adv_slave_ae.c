...
/*! \brief      Maximum value for 30usec offset unit. */
#define LCTR_30_USEC_OFFS_MAX_USEC      245730
...
static void lctrPackSyncInfo(lctrAdvSet_t *pAdvSet, uint8_t *pSyncInfo)
{
  ...
  if (offsUsec <= LCTR_30_USEC_OFFS_MAX_USEC)
  {
  ...
  temp = (LL_CHAN_DATA_ALL |                               /* All secondary channels are used. */
                                    ...
                                    uint8_t *pPduBuf, uint8_t commExtAdvPdu, bool_t isPeriodic)
{
      ...
      ((optExtHdrFlags & LL_EXT_HDR_ADI_BIT) && (!(pAdvSet->extHdrFlags & LL_EXT_HDR_ADI_BIT))))
  {
    ...
    UINT16_TO_BSTREAM(pBuf, (pAdvSet->param.advSID << 12) | ((pAdvSet->advData.did & 0x0FFF) << 0));
  ...
  availDataLen = LL_EXT_ADVB_MAX_LEN - (pBuf - pPduBuf);
      ...
      ((optExtHdrFlags & LL_EXT_HDR_SYNC_INFO_BIT) && (!(pAdvSet->extHdrFlags & LL_EXT_HDR_SYNC_INFO_BIT))))
  {
    ...
    availDataLen -= LCTR_SYNC_INFO_LEN;
  }
  ...
  if (pDataBuf)
  {
    ...
    if (pDataBuf->fragPref == LL_ADV_DATA_FRAG_ALLOW)
    {
  ...
  extHdrLen = pBuf - pPduBuf - LL_ADV_HDR_LEN - LCTR_EXT_HDR_CMN_LEN;
...
uint8_t lctrPackAdvExtIndPdu(lctrAdvSet_t *pAdvSet, uint8_t *pPduBuf, bool_t isPeriodic)
{
  ...
  prop = pAdvSet->param.advEventProp &
  ...
  advMode = ((pAdvSet->param.advEventProp & LL_ADV_EVT_PROP_CONN_ADV_BIT) ? LCTR_ADV_MODE_CONN_BIT : 0) |
...
{
  uint8_t manExtHdrFlags, optExtHdrFlags;
                               ...
                               bool_t isPeriodic)
{
  ...
  manExtHdrFlags = LL_EXT_HDR_ADI_BIT;
  optExtHdrFlags = LL_EXT_HDR_AUX_PTR_BIT | LL_ADV_EVT_PROP_TX_PWR_BIT;
...
uint8_t lctrPackAuxConnRspPdu(lctrAdvSet_t *pAdvSet, uint8_t *pPduBuf, bool_t isPeriodic)
{
  ...
  manExtHdrFlags = LL_EXT_HDR_ADV_ADDR_BIT | LL_EXT_HDR_TGT_ADDR_BIT;
...
uint8_t lctrPackLegacyAdvPdu(lctrAdvSet_t *pAdvSet, uint8_t *pPduBuf)
{
  ...
  /* Pack AdvData. */
  advDataLen = pAdvSet->advData.alt.legacy.len;
  switch (pduHdr.pduType)
  {
    case LL_PDU_ADV_IND:
    case LL_PDU_ADV_NONCONN_IND:
...
uint8_t lctrPackSyncIndPdu(lctrAdvSet_t *pAdvSet, uint8_t *pPduBuf, lctrAdvDataBuf_t *pAdvData, bool_t isPeriodic)
{
  ...
  manExtHdrFlags = 0;
  optExtHdrFlags = LL_EXT_HDR_AUX_PTR_BIT |
