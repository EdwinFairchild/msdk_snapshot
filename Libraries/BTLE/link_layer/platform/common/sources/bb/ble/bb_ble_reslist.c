...
uint16_t BbBleInitResolvingListSh(uint8_t numEntries, uint8_t *pFreeMem, uint32_t freeMemSize)
#else /* __CROSSWORKS */
...
uint16_t BbBleInitResolvingList(uint8_t numEntries, uint8_t *pFreeMem, uint32_t freeMemSize)
{
  _sdma_diag.pfnLastShCall = ((size_t)&BbBleInitResolvingListSh << 1);
  return BbBleInitResolvingListSh(numEntries, pFreeMem, freeMemSize);
}
#endif /* __CROSSWORKS */
