...
{
#ifdef ENABLE_BG_DELAY_LOOP

    unsigned int uPreWFIDelay = 4096U;
    unsigned int uPostWFIDelay = 4096U;

#endif /* ENABLE_BG_DELAY_LOOP */
...
#ifndef ENABLE_BG_DELAY_LOOP

    /* Go into Background mode and wait for an interrupt to wake the processor */
    __WFI();

#else /* ENABLE_BG_DELAY_LOOP */

#if defined(__CC_ARM) || defined(__ARMCC_VERSION)

    __WFI();

#else /* ! (__CC_ARM || __ARMCC_VERSION) */
    uPreWFIDelay ++;
    uPostWFIDelay ++;
